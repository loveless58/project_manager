...
| `agent`, phase 1 | Returns `status="awaiting_agent_judgement"`. | Request/input validation or preparation is blocked. | Creates only request/input-side artifacts: `input_manifest.json`, native extracted staging under `extracted/`, and `agent_judgement_requests.json`. There is no candidate, review, feedback, plan, or archive-gate artifact. |
| `agent`, phase 2 | A strict response is consumed and the candidate/review package is prepared. | The response is missing, malformed, mismatched, or already consumed. | Adds `agent_judgement_consumption.json`, `candidate_interpretations.json`, `archive_intents.json`, `review_queue.json`, `feedback_form.json`, `feedback_form.md`, `planned_archive_actions.json`, and `trace.json`. It does not run an archive gate or create `archive_execution_gate.json` / `archive_result.json`. |
