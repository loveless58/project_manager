...
def _render_review(document_result: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- 原始文件名：{document.get('original_filename', '')}",
        ...
        f"- 实际尝试：{json.dumps(parse.get('attempts', []), ensure_ascii=False, sort_keys=True)}",
        "",
        "## 结构化事实",
        "",
        "```json",
...
def _render_run_summary(
    ...
    lines = [
        ...
        f"- 运行 ID：{_run_id(run)}",
        f"- 目标：{run.get('goal', '')}",
        f"- 创建时间：{run.get('created_at_utc', '')}",
        f"- Agent：{run.get('agent', {}).get('name', '')} {run.get('agent', {}).get('version', '')}",
        "- 源文件是否修改：否",
