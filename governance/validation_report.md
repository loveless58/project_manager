...
- `项目文件/` 四阶段目录是否存在：`项目投标/`、`项目弃标/`、`项目丢标/`、`项目执行/`
...
- 实际注册的 37 个工具与 `governance/project_schema.json` 中声明的 37 个工具是否一致
...
- 数据清洗及文件整理 skill 暴露 16 个工具，其中 `extract_structured_business_output` 负责只读结构化证据输出，`semantic_structure_document` 负责 evidence pack 的只读语义结构化；`run_ocr`、`prepare_file_organization_run`、`apply_human_review`、`execute_archive_plan` 共同构成 OCR 处理与文件整理闭环。
